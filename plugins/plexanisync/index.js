...
async function register(api) {
  ...
  if (pipTest.error) {
    api.communication.logError(
      `Could not register plex anilist sync, pip does not seem to be installed or does not work correctly. ${pipTest.error?.toString()}`,
    );
    return;
  }
  ...
  if (!fs.existsSync(plexAniSyncMasterPath)) {
    api.communication.logInfo(
      `Loading PlexAniSync from https://github.com/RickDB/PlexAniSync/archive/master.zip`,
    );
    await api.downloadPluginResourceZipAndUnzip(
      api.thisPluginDirectory,
    ...
    const result = child_process.spawnSync(
      ...
      { cwd: plexAniSyncMasterPath },
    );
    if (result.status !== 0) {
      fs.rmSync(plexAniSyncMasterPath, { recursive: true, force: true });
      throw new Error(
        `Error while installing dependencies for anilist sync ${result.stderr?.toString()}`,
      );
    }
    fs.writeFileSync(
      `${plexAniSyncMasterPath}/settings.ini`,
      getPlexAniSyncTemplate({}),
    );
...
function syncAniList(api) {
  ...
  const process = child_process.spawn(pythonExecutable, ["PlexAniSync.py"], {
    cwd: plexAniSyncMasterPath,
  });
  ...
  process.on("exit", (code) => {
    if (code === 0) {
      ...
      api.communication.logError(
        `Error while syncing to anilist. For more information see "${api.thisPluginDirectory}/error.log"`,
      );
...
async function onConfigUpdate(api, config) {
  fs.writeFileSync(
    `${api.thisPluginDirectory}/PlexAniSync-master/settings.ini`,
    getPlexAniSyncTemplate(config),
  );
  if (intervalHandler) {
  ...
  intervalHandler = setInterval(
    () => syncAniList(api),
    1000 * 60 * config.sync_interval_in_minutes,
  );
}
...
plex_episode_count_priority = ${
    config.plex_episode_count_priority ? "True" : "False"
  }
...
var index = {
  ...
  pluginConfigurationDefinition: [
    ...
    { key: "authentication_method_direct", type: "boolean", default: true },
    {
      ...
      type: "text",
      default: "",
      enableWhen: { key: "authentication_method_direct", is: true },
    },
    {
      key: "token",
      type: "text",
    ...
    },
    {
      ...
      type: "text",
      default: "",
      enableWhen: { key: "authentication_method_direct", is: false },
    },
    {
      key: "myplex_user",
      type: "text",
    ...
    { key: "home_user_sync", type: "boolean", default: false },
    {
      ...
      type: "text",
      default: "",
      enableWhen: { key: "home_user_sync", is: true },
    },
    {
      key: "home_server_base_url",
      type: "text",
      default: "",
      enableWhen: { key: "home_user_sync", is: true },
    },
